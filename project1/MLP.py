...
class MLP(nn.Module):
    ...
    def forward(self,x):
        ...
        Input -> Linear1 -> Batchnorm1d1 -> Linear2 -> BatchNorm1d2 -> Linear3
        """
        x = self.bn1(F.relu(self.fc_1(x.view(-1,392))))
        x = self.bn2(F.relu(self.fc_2(x)))
        x = self.fc_3(x)
        return x

"""
Train MLP model with cross entropy loss and SGD
    lr: stands for learning rate
    nb_epochs: stands for the number of epochs in a training process
"""    
def train_model(model, train_input, train_classes, train_target, mini_batch_size = 100, lr = 0.01, nb_epochs = 25):
...
"""   
def evaluate_model(model_class, train_iter = 20, size = 1000, mini_batch_size = 100, lr = 0.005, nb_epochs = 25):
    losses_vals = []
    train_accuracys = []
    test_accuracys = []
    for n in range(train_iter):
        ...
        #train the model
        model.train(True)
        losses = train_model(model, train_input, train_classes, train_target, mini_batch_size, lr, nb_epochs)
